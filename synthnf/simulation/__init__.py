...
class RandomParameters():    
    def __init__(
        ...
        *,
        ...
        self.seed = seed if seed is not None else np.random.randint(np.iinfo(np.int32).max)
        ...
        self.grid_tops_mm = grid_tops_mm or np.array(defaults.blueprints.fa.grid_tops_mm)
        self.rods_per_face = rods_per_face or defaults.blueprints.fa.rods_per_face
        
        self.rod_centers = rod_centers if rod_centers is not None else frm.generate_rod_centers(
            self.rods_per_face,
            self.rod_width_mm,
            self.rod_gap_mm
        )
    ...
    def rod_material(self):
        ...
        blend_texture = textures.blend(noise,.2,self.cloudiness)
